...
static void gfx_opengl_handheld_ensure_fbo(uint32_t window_w, uint32_t window_h) {
    ...
    // Low-Res mode renders the 3D world through the low-res upscale FBO.
    // With it off the world renders straight to the window at its native
    // resolution. The HUD pass and blit are both no-ops in this state,
    // so this is a live toggle.
    if (!configHandheldLowres) {
